...
- A running scout shows the named current activity, its provider and query, and how recently the run record changed. Long runs must never present only a spinner; live counters, provider attempts, and retrieved candidates update while retrieval is still in progress.
